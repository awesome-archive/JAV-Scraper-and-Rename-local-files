...
class JavWeb(object):
    ...
    def __init__(self, settings: Ini, appoint_symbol: str, headers: Dict[str, str]):
        ...
        self._website = self.__class__.__name__
        """当前类名，表示处理模式，例如“JavBus”"""

        if self._website == 'JavWeb':
            raise Exception("<JavWeb>不能被实例化")

        tuple_temp: tuple[str, dict] = settings.web_url_proxy(self._website)
        ...
        self._DICT_GENRES = better_dict_genres(self._website, settings.to_language)
        """优化后的genres"""
    ...
    def _appoint(self, jav_file: JavFile):
        ...
        if not item_appointg:
            # 指定的格式不正确
            raise SpecifiedUrlError(f'你指定的网址的格式有问题: {self._website} {jav_file.Name}')
    ...
    def _request_url(self, url: str, allow_redirects=True):
        try:
            return self._requests.get(url, timeout=(36, 42), allow_redirects=allow_redirects)
        except ProxyError:
    ...
    def _get_html(self, aim: str, url: str):
        ...
        """
        # library不需要跳转
        allow_redirects = (self._website != 'JavLibrary')
        ...
        for _ in range(4):
            ...
            if self._confirm_normal_rsp(rsp_content):
                ...
                # 被网站封禁ip
                input(f'    >被{self._website}网站封禁: {rsp_content}')
            print('    >打开网页失败，空返回...重新尝试...')
            print(rsp_content)
        # Todo 删除
        # raise
        input(f'>>请检查你的网络环境能否打开: {url}')

    def download_picture(self, url: str, path: str):
        """下载图片"""

        if not url:
            print(f'    >{self._website}没找到图片')
            return False

        if self._website == 'JavBus':
            url = f'{self._URL}/pics/cover/{url}'

        print(f'    >从{self._website}下载封面:', url)
        for _ in range(3):
            ...
            return True
        print(f'    >从{self._website}下载图片失败')
        return False

    def _select_normal(self, jav_data: JavData):
        """（找到目标网页后）更新信息"""
        # 更新网站对应的item
        setattr(jav_data, self._website, self._item)
